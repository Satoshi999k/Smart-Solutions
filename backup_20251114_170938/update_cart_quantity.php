...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $itemIndex = isset($_POST['item_index']) ? (int)$_POST['item_index'] : 0;
    $action = isset($_POST['action']) ? $_POST['action'] : '';

    // Define the products array (same as in cart.php)
    $products = [
    ...
    // Debug logging
    error_log("Item Index: $itemIndex, Action: $action, Cart Count: " . count($cart));
    
    if (!empty($cart) && isset($cart[$itemIndex])) {
        $cartItem = $cart[$itemIndex];
        $currentQuantity = isset($cartItem['quantity']) ? $cartItem['quantity'] : 1;

        // Find the product in the $products array to get its price
        $productInfo = array_filter($products, function($p) use ($cartItem) {
            return $p['id'] == $cartItem['id'];
        });
        $productInfo = reset($productInfo); // Get the first matched product
        ...
        if ($productInfo) {
            ...
            if ($currentQuantity > 0) {
                $_SESSION['cart'][$itemIndex]['quantity'] = $currentQuantity;
                $newSubtotal = $productInfo['price'] * $currentQuantity;
                
                // Update database if user is logged in
                if (isset($_SESSION['user_id'])) {
                    $user_id = $_SESSION['user_id'];
                    $product_id = $cartItem['id'];
                    $updateDbQuery = "UPDATE shopping_cart SET quantity = ? WHERE user_id = ? AND product_id = ?";
                    $updateDbStmt = $conn->prepare($updateDbQuery);
                    if ($updateDbStmt) {
                        $updateDbStmt->bind_param("iii", $currentQuantity, $user_id, $product_id);
                        $updateDbStmt->execute();
                        $updateDbStmt->close();
                    }
                }
            } else {
                ...
                if (isset($_SESSION['user_id'])) {
                    $user_id = $_SESSION['user_id'];
                    $product_id = $cartItem['id'];
                    $deleteDbQuery = "DELETE FROM shopping_cart WHERE user_id = ? AND product_id = ?";
                    $deleteDbStmt = $conn->prepare($deleteDbQuery);
                    if ($deleteDbStmt) {
                        $deleteDbStmt->bind_param("ii", $user_id, $product_id);
            ...
            // Recalculate total cart price and total items
            $cartTotal = 0;
            $totalItemsInCart = 0;
            foreach ($_SESSION['cart'] as $item) {
                $p = array_filter($products, function($prod) use ($item) {
                    return $prod['id'] == $item['id'];
                });
                $p = reset($p);
                if ($p) {
                    $itemQuantity = isset($item['quantity']) ? $item['quantity'] : 1;
                    $cartTotal += $p['price'] * $itemQuantity;
                    $totalItemsInCart += $itemQuantity;
                }
            }

            $response['success'] = true;
            $response['new_quantity'] = $currentQuantity;
            $response['new_subtotal'] = $newSubtotal;
            $response['cart_total'] = $cartTotal;
            $response['total_items_in_cart'] = $totalItemsInCart;
        } else {
            $response['message'] = 'Product not found in defined products list.';
        }
    } else {
