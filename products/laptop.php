...
$result = $conn->query("SELECT id, name, price, image, stock FROM products WHERE LOWER(category) = 'laptop' ORDER BY id DESC");
$products = [];
...
if ($result && $result->num_rows > 0) {
    ...
    error_log("DEBUG laptop.php: No laptops found. Checking categories...");
    $cat_result = $conn->query("SELECT DISTINCT LOWER(category) as cat FROM products");
    if ($cat_result) {
        while ($cat_row = $cat_result->fetch_assoc()) {
            error_log("  Found category: '" . $cat_row['cat'] . "'");
        }
    }
    
    // Try fetching ALL products to see what's in database
    $all_result = $conn->query("SELECT id, name, category FROM products LIMIT 5");
    if ($all_result && $all_result->num_rows > 0) {
        while ($all_row = $all_result->fetch_assoc()) {
            error_log("  Sample: ID={$all_row['id']}, Name={$all_row['name']}, Category='{$all_row['category']}'");
        }
    }
}
...
<div class="menu">
    ...
    <a href="../products/laptop.php" style="font-weight: bold;">LAPTOP</a>
...
<div class="breadcrumb">
    ...
    <a><i class="material-icons" style="font-size: 16px; vertical-align: middle; margin-right: 4px;">laptop</i>Laptop</a>
</div>
...
<div class="product-grid">
    ...
    if (!empty($products)) {
        ...
        echo "<p style='grid-column: 1/-1; text-align: center;'>No laptops available at the moment.</p>";
    }
    ?>
</div>
